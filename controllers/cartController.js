...
const {
  ...
  checkCartHasThisProductId,
  checkUserExistInUser,
...
module.exports.addToCart = async (req, res) => {
  ...
  try {
    ...
    if (cartID) {
      //<< if cart id is present

      //
      for (const item of Object.values(cartItems)) {
        //<< loop to check cartItems
        ...
        const isDuplicate = await checkCartHasThisProductId(
          req,
          res,
          cartID,
          itemProduct
        ...
        if (isDuplicate) {
          ...
          const existingItemIndex = cart.cartItems.findIndex(
            (itemIndex) =>
              itemIndex.productId.toString() === itemProduct.toString()
          );
...
const addToCartNormal = async (req, res, userId, cartItems) => {
  ...
  try {
    ...
    totalPrice = Number(
      updatedItems.reduce((sum, item) => sum + item.subtotal, 0).toFixed(2)
    );
